...
import { autonomouslyApproveAndRequestAgent, pollActiveAgents } from "@/lib/autoapp/execute";
...
async function observe() {
  try {
    const polled = await pollActiveAgents();
    ...
    return NextResponse.json({ ...result, polledAgents: polled });
  } catch (error) {
