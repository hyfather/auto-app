...
export async function summarizeLatestCycle() {
  ...
  return `[${code}] Result\nStatus: ${cycle.status}.\nMission: ${cycle.mission.title}.\nProposal: ${cycle.proposal}.\nLatest related updates:\n${updates}\nNext step: ${cycle.status === "proposed" ? "AutoApp can approve and execute autonomously; use reject/pause only if you want to stop it." : "Keep watching #general for GitHub/Vercel/Codex updates."}`;
}
